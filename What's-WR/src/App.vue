  ...
  methods: {
        ...
        */
        ...
        .then((response) => {
            if(response.length < 1000) { // If less than 1,000 games are found...
                app.totalNumOfGames = app.totalNumOfGamesStartingOffset + response.length;

                // Begin finding random runs to show the user.
                // app.getNewRuns();
            } else {
                // Add 1,000 to the starting offset and run this function again.
                app.totalNumOfGamesStartingOffset += 1000;
                app.getTotalNumOfGames();
            }
        });
    },
    getNewRuns: function() {
        var target = app.targetNumOfBackups;
        if(!app.displayedRun) { target++; }
    ...
    getNewRun: function() {
        ...
        .then((response) => {
            ...
            masterArray = app.getRandomSetOfCategories(masterArray);
            // Fetch the world records for every category within masterArray.
            return app.getRecordsFromCategoryArray(masterArray);
        }).then((response) => {
            ...
            masterArray = masterArray.filter((category) => {
                ...
                return app.twitchRegEx.test(videoURL) || app.ytRegEx.test(videoURL);
            });
            ...
            result = masterArray[app.getRandomNumber(masterArray.length - 1)];

            // Fetch information about the player(s).
            return app.getAllPlayersInfo(result);
        }).then((response) => {
            ...
            if(!app.displayedRun) {
                ...
                app.setDisplayedRun(newObj);
            } else { app.backupRuns.push(newObj); }
        }).catch(() => {
    ...
    getRecordsFromCategoryArray: function(arrayOfCategories) {
        ...
        arrayOfCategories.forEach((category) => {
            ...
            promises.push(app.makeAsyncCall(url));
        });
    ...
    setDisplayedRun: function(newRunObj) {
        ...
        if(!newRunObj) {
            ...
            app.displayedRun = null;

            // Start the idle timeout.
            app.startIdleTimeout(app.idleTimeoutDuration);
        } else {
            // If the page was idle before, reset it.
            this.pageIsIdle = false;

            // Move the new run into displayedRun.
            app.displayedRun = newRunObj;

            // Update the location hash
            window.location.hash = encodeURIComponent(app.displayedRun.id);
        }
    },
    getAllPlayersInfo: function(categoryObj) {
        ...
        categoryObj.run.players.forEach((player) => {
            if(player.rel === 'guest') {
                ...
                promises.push(app.makeAsyncCall(player.uri));
            }
        });
    ...
    getRunFromHash: function() {
        ...
        // Once the run is returned, execute app.getTotalNumOfGames in order to kick off the normal code flow.
    ...
    startIdleTimeout: function(duration) {
        ...
        setTimeout(function() {
            if(!app.displayedRun) { app.pageIsIdle = true; }
        }, duration * 1000);
    }
  },
  created: function() {
